        ...
        try:
            ...
            qa_prompt = ChatPromptTemplate.from_messages(
                [
                    ...
                    ("human", "Answer use markdown : {question}"),
                ]
            )
    ...
    def get_response(self, user_query, received_time, pasted_flag):
        try:
            ...
            response = self.chain_with_history.invoke(
                {"context": contents, "question": rewrite_query},
                config={"configurable": {"session_id": self.session_id}}
            )
            print()
            print(response)
            ...
            for message in stored_messages[:-3]:
                self.store[self.session_id].add_message(message)
            self.store[self.session_id].add_message(stored_messages[-1])

            self.save_history_to_pickle()
            self.save_history_to_csv(user_query, rewrite_query, contents, response, received_time, response_time, pasted_flag)
